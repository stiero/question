...
def frequency_parser(file):
    ...
    for i, line in enumerate(file):
        if "FREQUENCY" in line and line.startswith("FREQUENCY"):
            line = line.replace(",", "")
            store['FREQUENCY_' + str(i)] = line.strip() #Here, the key is a generated alias to maintain uniqueness of dictionary keys     
    return store
        ...
        for sub_coll in coll:
            for i, line in enumerate(sub_coll):
                if i % 2 == 0:
                    ...
                    sub_coll[i] = str(sub_coll[i] + "," + sub_coll[i+1])
                    sub_coll[i+1] = None
            
            """ This is to remove duplicate rows, now that we have merged magnitude and phase rows as one.
                Tried doing the below steps in the loop above, but it messed with the indices.
                Hence, I am repeating the loop here.          
            """
            for i, line in enumerate(sub_coll):
                if line == None:
                    del sub_coll[i]
...
columns = [["point_id", "type", "t1_magnitude", "t2_magnitude", "t3_magnitude",
           ...
           "t3_phase", "r1_phase", "r2_phase", "r3_phase"]]
